                    ...
                    <div className="max-w-7xl mx-auto px-2">
                        <div className="flex items-center gap-3 overflow-x-auto py-2">
                            {SECTIONS.map((section, index) => {
                                ...
                                const isBefore = index < currentSectionIndex;
                                return (
                                    <button
                                        key={section.id}
                                        type="button"
                                        onClick={() => goToSection(section.id)}
                                        className={`flex items-center gap-3 px-3 py-2 rounded-lg whitespace-nowrap text-sm font-medium transition-all duration-200 focus:outline-none ${
                                            isActive
                                        ...
                                        style={isActive ? { border: 'none', backgroundColor: sectionColors.primary, color: '#fff' } : {}}
                                    >
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold ${
                                            isActive ? 'bg-indigo-600 text-white' : isBefore ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-600'
                                        }`}>
                                            {index + 1}
                                        </div>
                                        <div className="hidden sm:block text-left">
                                            <div className="text-sm font-medium leading-tight">{section.name}</div>
                                            <div className="text-xs text-gray-400">{section.id}</div>
                                        </div>
                                    </button>
                                );
                            })}
